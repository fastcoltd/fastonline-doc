...
const aftersalesConfig = [
    {
        name: "交付与可用性问题",
        reasons: [
            "未按时交付（如账号未到账、软件未提供下载链接）",
            "交付信息错误（如密码无效、兑换码已使用）",
            "商品或服务被重复出售（如账号已被他人使用）",
            "账号无法登录（如被封禁、绑定信息缺失）",
            "服务中断或不稳定（如云服务宕机、VPN掉线）",
            "软件崩溃或存在严重BUG",
            "卖家未提供激活码或使用说明",
            "服务开通延迟（如会员未激活、SEO未启动）",
            "物流问题（若涉及实体卡或U盘等）",
            "需要额外验证或配置但未告知"
        ]
    },
    {
        name: "商品或服务质量不符",
        reasons: [
            "属性与描述不符（如账号等级、虚拟货币数量不足）",
            "服务效果未达预期（如SEO排名未提升、流量虚假）",
            "软件功能缺失或未按需求实现",
            "文章内容空洞或与标题不符",
            "需求发布成果质量低（如设计稿、代码未达标）",
            "稀有度或价值被夸大（如账号稀有皮肤被虚构）",
            "服务周期或支持范围缩水（如仅短期有效）",
            "代码质量差（如未优化、漏洞多）",
            "兼容性问题（如软件无法在指定系统运行）",
            "报告数据不准确或伪造（如SEO效果报告）"
    ...
    },
    {
        name: "欺诈与信任问题",
        reasons: [
            "商品或服务不存在（如虚假兑换码、虚拟账号）",
            "账号被卖家找回（如使用原绑定信息取回）",
            "卖家提供伪造证明或截图（如流量数据造假）",
            "服务违反规则导致处罚（如黑帽SEO被封）",
            "接单方无能力完成任务（如需求发布诈骗）",
            "账号存在未披露的违规记录（如限流、封号风险）",
            "虚假宣传效果（如文章付费承诺收益不实）",
            "卖家失联或跑路",
            "交易后信息泄露（如账号被盗用）",
            "诱导消费后不履行承诺"
...
let memberMenuConfig = [
    { text: "店铺管理", style: `color:var(--natural-green)`, icon: "fas fa-store", show: () => userHasStore(), memberSidebar: false, storeSidebar: true, href: "/site/store/overview.html", sub: [
            ...
            { text: "提现管理", icon: "fas fa-money-check-alt", href: "/site/store/withdraw.html"},
            { text: "订单日志", icon: "fas fa-box-open", href: "/site/store/order-logs.html" },
            { text: "资金记录", icon: "fas fa-money-check-alt", href: "/site/store/money-logs.html"},
            { text: "博客设定", icon: "fas fa-blog", href: "/site/store/blog.html" },
